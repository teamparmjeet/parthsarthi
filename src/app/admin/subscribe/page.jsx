...
export default function Page() {
    ...
    useEffect(() => {
        const alldata = async () => {
            try {
                const response = await axios.get('/api/subscribe/fetchall/we');
    ...
    const handleDelete = async (_id) => {
        try {
            ...
            const response = await axios.delete(`/api/subscribe/delete?_id=${_id}`);

            if (response.status === 200) {
                alert('Subscribe deleted successfully.');
            ...
            alert('Error deleting the Subscribe.');
        }
