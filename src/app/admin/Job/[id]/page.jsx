...
export default function Page({ params }) {
    ...
    useEffect(() => {
        const dataGet = async () => {
            try {
                const response = await axios.get(`/api/job/${contactId}`);
    ...
    const handleDelete = async () => {
        try {
            setLoading(true);
            const response = await axios.delete(`/api/job/delete?_id=${contactId}`);
        ...
        <div className="container lg:w-[90%] mx-auto py-5">
                ...
                <div className="flex space-x-3">
                    <Link href={"/admin/Job"}>
                        <button className="bg-[#29234b] rounded-md flex items-center text-white text-sm px-4 py-2 ">
                            <ArrowLeft size={16} className="me-1" /> All Enquiry
                        </button>
                    </Link>
                    <button
            ...
            <div className="bg-white shadow-lg overflow-hidden border border-gray-200">
                ...
                <div className="grid grid-cols-12 gap-4 p-5">
                    ...
                    <div className="sm:col-span-12 col-span-12">
                        <label htmlFor="jobprofile" className="block text-[12px] text-gray-700">
                            Job Profile
                        </label>
                        <input
                            type="text"
