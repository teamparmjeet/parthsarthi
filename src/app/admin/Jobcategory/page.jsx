...
export default function Page() {
    ...
    const filtereddata = sortdata(
        data.filter(dat =>
            dat.category.toLowerCase().includes(searchTerm.toLowerCase())
        )
    );
    ...
    const handleDelete = async (_id) => {
        try {
            ...
            const response = await axios.delete(`/api/jobcategory/delete/${_id}`);
                            ...
                            currentdata.map((item) => (
                                ...
                                >
                                    <td className="px-4 py-2 group">
                                        <Link className="font-semibold text-gray-900 text-sm whitespace-nowrap" href={`/admin/Jobcategory/edit/${item._id}`}>{item.category}</Link>
                                        <div className="flex mt-1 opacity-0 group-hover:opacity-100">
                                            <Link className="text-[11px] text-sky-900 pr-2 border-r leading-none pt-[1px] pb-[2px] border-r-neutral-500" href={`/admin/Jobcategory/edit/${item._id}`}>View</Link>
                                            <Link className="text-[11px] text-sky-600 px-2 border-r leading-none pt-[1px] pb-[2px] border-r-neutral-500" href={`/admin/Jobcategory/edit/${item._id}`}>Edit</Link>
                                            <button
